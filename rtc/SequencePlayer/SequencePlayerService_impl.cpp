...
CORBA::Boolean SequencePlayerService_impl::setJointAnglesSequenceFullWithBSpline(CORBA::Short bsorder, CORBA::Short bsid, CORBA::Double bstmin, CORBA::Double bstmax, const dSequence& bsp, const dSequenceSequence& jvss, const dSequenceSequence& vels, const dSequenceSequence& torques, const dSequenceSequence& poss, const dSequenceSequence& rpys, const dSequenceSequence& accs, const dSequenceSequence& zmps, const dSequenceSequence& wrenches, const dSequenceSequence& optionals, const dSequence &tms)
{
  ...
  if (bsid <= 0) {
      std::cerr << __PRETTY_FUNCTION__ << " bspline id max is invalid:" << bsid << " > 0" << std::endl;
  }
  if (bstmin < 0.0) {
  ...
  if (bsp.length() <= 0) {
      std::cerr << __PRETTY_FUNCTION__ << " num of bspline p is invalid: start is" << bsp.length() << std::endl;
  }
  if (jvss.length() <= 0) {
      std::cerr << __PRETTY_FUNCTION__ << " num of joint angles sequence is invalid:" << jvss.length() << " > 0" << std::endl;
      return false;
...
/*
CORBA::Boolean SequencePlayerService_impl::setJointAnglesSequenceFullTest(const dSequenceSequence& jvss, const dSequenceSequence& vels, const dSequenceSequence& torques, const dSequenceSequence& poss, const dSequenceSequence& rpys, const dSequenceSequence& accs, const dSequenceSequence& zmps, const dSequenceSequence& wrenches, const dSequenceSequence& optionals, const dSequence &tms)
{
  if (jvss.length() <= 0) {
      std::cerr << __PRETTY_FUNCTION__ << " num of joint angles sequence is invalid:" << jvss.length() << " > 0" << std::endl;
      return false;
  ...
  return m_player->setJointAnglesSequenceFullTest(jvss, vels, torques, poss, rpys, accs, zmps, wrenches, optionals, tms);
}
*/
